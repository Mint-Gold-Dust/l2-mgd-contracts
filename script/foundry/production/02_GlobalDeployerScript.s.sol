...
contract GlobalDeployerScript is FileSystem, MgdScriptConstants {
  ...
  function executeDeployActions(bool marketPlaceWithVouchers) internal {
    ...
    if (config.company == Action.DEPLOY) {
      ...
      MgdCompanyL2Sync company = MgdCompanyL2SyncDeployer.deployMgdCompanyL2Sync(fs, params, true);
    ...
    if (config.mgd721 == Action.DEPLOY) {
      ...
      MgdERC721PermitEscrowableDeployer.deployMgdERC721PermitEscrowable(fs, params, true);
    }
    ...
    if (config.mgd1155 == Action.DEPLOY) {
      ...
      MgdERC1155PermitEscrowableDeployer.deployMgdERC1155PermitEscrowable(fs, params, true);
    }
    // Mgd721L2Voucher
    ...
    if (config.mgdSetPrice == Action.DEPLOY) {
      ...
      MintGoldDustSetPriceDeployer.deployMintGoldDustSetPrice(fs, params, true);
    }
    ...
    if (config.mgdAuction == Action.DEPLOY) {
      ...
      MintGoldDustMarketplaceAuctionDeployer.deployMintGoldDustMarketplaceAuction(fs, params, true);
    }
  }
